...
- `queueExecutor.ipairs(tab: { [a]: b }, fncExec: (any) -> any, andThen: (any) -> any)`: This acts like the above `pairs` function, except for each iteration, it returns the number of the current iteration instead of the current key. This may look like: 
...
- `queueExecutor.doWhile(condition: (any) -> any, fncExec: (any) -> any, andThen: (any) -> any)`: This is like the above function, except it will always execute the internal of the loop atleast once. This may look like: 
